...
namespace Beavis.Ipc
{
    public class NamedPipeServer : IDisposable
    {
        ...
        public event EventHandler<PipeMsgEventArgs> OnRequestReceived = delegate { };
        ...
        private void NewServerInstance()
        {
            ...
            if (_servers.Count < _options.MaxNumberOfServerInstances)
            {
                ...
                server.OnConnected += (s, e) => NewServerInstance();

                server.newRequestEvent += (s, e) => OnRequestReceived.Invoke(s, e);
            ...
            else
            {
                for (int i = _servers.Count - 1; i >= 0; i--)
                {
                    if (_servers[i] == null)
                    {
                        ...
                        (_servers[i].Communication.Status == TaskStatus.RanToCompletion ||
                        _servers[i].Communication.Status == TaskStatus.Canceled ||
                        _servers[i].Communication.Status == TaskStatus.Faulted))
                    {
                        _servers[i].Dispose();
                        _servers.RemoveAt(i);
                    }
